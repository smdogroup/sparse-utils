...
//  *
//  * Note: ptrdiff_t is preferable over int because int is often 32 bits and
//  might
//  * not be large enough for 64-bit environments. ptrdiff_t is also preferable
//  * over size_t because it is signed.
//  *
//  * Caveat: If the array size is so large (greater than PTRDIFF_MAX but less
//  than
//  * SIZE_MAX), element indices may not be representable by ptrdiff_t and the
//  * behavior is undefined. But usually ptrdiff_t is 64-bit which is
//  sufficient.
//  *
//  * Refs:
...
using index_t = int;  // we use int for now because that's what lapack uses...
